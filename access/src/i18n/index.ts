...
// 导入组件包的i18n
import settingsI18n from '@zhqc-smart/settings/i18n';
import adminI18n from '@zhqc-smart/admin/i18n';
...
// 合并组件包的i18n
if (settingsI18n) {
	for (const lang in settingsI18n) {
		if (itemize[lang]) {
			itemize[lang].push(...settingsI18n[lang]);
		}
	}
}
if (adminI18n) {
	for (const lang in adminI18n) {
		if (itemize[lang]) {
			itemize[lang].push(...adminI18n[lang]);
		}
	}
}
