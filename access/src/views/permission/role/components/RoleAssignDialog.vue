...
const filteredAvailableUsers = computed(() => {
	...
	return availableUsers.value.filter((user) =>
		user.name.includes(availableSearchText.value)
	);
});
...
const filteredAssignedUsers = computed(() => {
	...
	return assignedUsers.value.filter((user) =>
		user.name.includes(assignedSearchText.value)
	);
});
...
const moveToAvailable = () => {
	const selectedUsers = assignedUsers.value.filter((user) =>
		selectedAssignedIds.value.includes(user.id)
	);
	availableUsers.value = [...availableUsers.value, ...selectedUsers];
	assignedUsers.value = assignedUsers.value.filter(
		(user) => !selectedAssignedIds.value.includes(user.id)
	);
	selectedAssignedIds.value = [];
};
