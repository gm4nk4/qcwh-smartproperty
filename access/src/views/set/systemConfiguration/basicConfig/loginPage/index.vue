		...
		<el-col :span="20" :offset="2" style="height: 100%; overflow: auto">
			<el-card>
				<template #header>
					<el-row>
						<el-col :span="24">
							...
							<el-button @click="reset" icon="Refresh" :loading="loading">恢复默认设置</el-button>
							<el-button @click="formDialogRef.openDialog(form.templateId)" icon="view">登录页效果预览</el-button>
						</el-col>
					</el-row>
				</template>
				...
				<div class="login-box-con">
					<div class="left">
						<TemplateReview v-if="tabValue == '登录缩略图'" :templateId="form.templateId" :scale="0.4" />
						<NewCodeEditor
							v-else
							:height="`${100 * 0.4}vh; `"
						...
						/>
						<div class="tab-box">
							<el-segmented v-model="tabValue" :options="tabOptions" size="default" />
						</div>
					</div>
					<div class="right">
						<el-form ref="dataFormRef" :model="form" :rules="dataRules" label-width="140px" v-loading="loading">
							...
							<el-form-item label="登录页功能" prop="menuDisplay">
								<el-select placeholder="请选择功能" multiple v-model="form.menuDisplay">
									<el-option :key="index" :label="item.label" :value="item.value" v-for="(item, index) in menuOptions"></el-option>
								</el-select>
							</el-form-item>
							...
							<el-form-item label="登录logo图片" prop="middleLogoUrl">
								<el-row :gutter="24">
									<el-col :span="12">
										<UploadImg class="systemSmallLogo" v-model:imageUrl="form.middleLogoUrl">
											<template #empty>
												<span>图标48*48</span>
											</template>
										</UploadImg>
							...
							<el-form-item label="登录页背景" prop="backgroundUrl">
								<UploadImg v-model:imageUrl="form.backgroundUrl" class="systemBackground">
									<template #empty>
										<span>登录页背景</span>
									</template>
								</UploadImg>
...
import { templateData, templateDrawDownList, templateFormData } from './indexConfig';
import { updateTemplate, getTemplateList } from '/@/api/systemConfiguration/basicConfig';
import { storeToRefs } from 'pinia';
import { Local } from '/@/utils/storage';
import { useThemeConfig } from '/@/stores/themeConfig';
// 引入组件
const FormDialog = defineAsyncComponent(() => import('./form.vue'));
const ColorPicker = defineAsyncComponent(() => import('../../../../../components/ColorPicker/index.vue'));
const NewCodeEditor = defineAsyncComponent(() => import('/@/components/newCodeEditor/index.vue'));
const TemplateReview = defineAsyncComponent(() => import('./component/templateReview.vue'));
...
const dataRules = ref({
	...
	backgroundColor: [{ required: true, message: '登录背景颜色', trigger: 'blur' }],
	middleLogoUrl: [{ required: true, message: '登录logo图片不能为空', trigger: 'blur' }],
	backgroundUrl: [{ required: true, message: '登录页背景图片不能为空', trigger: 'blur' }],
	copyRightDisplay: [{ required: true, message: '请选择登录底部版权信息', trigger: 'blur' }],
});
...
const getDatasourceConfData = () => {
	...
	getTemplateList({ type: '2' }).then((res: any) => {
		...
		let obj = formList.value.filter((item: any) => item.isSelected == 1) as any[];
		if (obj.length) {
			setFormValue(obj);
...
const setFormValue = (obj: any, key?: any) => {
	if (obj.length) {
		...
		form.templateScript = form.templateScript ? JSON.parse(form.templateScript) : '';
	} else {
		Object.assign(form, templateFormData.filter((item) => item.templateId == key)[0]);
	}
...
//恢复默认设置
const reset = async () => {
	Object.assign(form, templateFormData.filter((item) => item.templateId == form.templateId)[0]);
	onSubmit();
};
</script>
