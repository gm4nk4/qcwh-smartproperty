		...
		<el-dialog :title="dataForm.id ? $t('common.editBtn') : $t('common.addBtn')" v-model="visible" width="600">
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button @click="onSubmit" type="primary" :loading="loading">{{ $t('common.confirmButtonText') }}</el-button>
				</span>
			</template>
		</el-dialog>
	</div>
...
const onSubmit = async () => {
	...
	try {
		loading.value = true;
		const res = dataForm.id ? await putObj(dataForm) : await addObj(dataForm);
		if (res.data) {
