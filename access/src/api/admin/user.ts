...
export const listByMsgWay = (way?: number | string) => {
	const hasWay = way !== undefined && way !== null && String(way).trim() !== '';
	const url = hasWay ? `/admin/user/listByMsgWay?way=${encodeURIComponent(String(way))}` : '/admin/user/listByMsgWay';
	return request({
		url,
		method: 'post',
	});
