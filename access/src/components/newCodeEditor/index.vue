...
const theme = ref('idea');
const themeList = [
	{ value: 'idea', color: '#444444', background: '#f7f7f7' },
	{ value: 'darcula', color: '#a9b7c6', background: '#2b2b2b' },
];
...
const normalizeHeight = (height: string): string => {
	const value = String(height || '').trim().replace(/;+$/, '');
...
onMounted(() => {
	nextTick(() => {
		...
		// 初始化高度（编辑器高度+工具栏高度）
		editorHeight.value = normalizeHeight(props.height);
