...
import { syncSubAppRoute } from '/@/qiankun/actions';

import { useQiankun } from '/@/hooks/useQiankun';
let { inQiankun } = useQiankun();
...
export const router = createRouter({
	history: inQiankun ? createMemoryHistory() : createWebHashHistory(),
	/**
	 * 说明：
...
router.beforeEach(async (to, from, next) => {
	// 子应用路由同步
	let targetPath = syncSubAppRoute(to, from);
	...
	if (to.meta.isAuth !== undefined && !to.meta.isAuth) {
		...
		if (!token) {
			...
			if (routesList.value.length === 0) {
				...
				// 路由初始化完成后，使用 replace 确保不会留下历史记录
