...
export const getChartColor = (index: number) => {
	...
	const chartColors = storesThemeConfig.themeConfig.currentTheme?.color?.chart || ['#1677FF', '#00CAE8', '#E98A1D', '#E8BB19', '#BD74E6'];
	return chartColors[index % chartColors.length];
};

// 获取渐变图表颜色的公共方法
export const getGradientChartColor = (index: number) => {
	const storesThemeConfig = useThemeConfig();
	const gradientChartColors = storesThemeConfig.themeConfig.currentTheme?.color?.gradientChart || ['#D9E3ED', '#E6F5F8', '#EDE2D9', '', '', '#FBFDFF', '#EDD9D9'];
	return gradientChartColors[index % gradientChartColors.length];
};
