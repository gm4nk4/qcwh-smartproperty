...
import { applyThemeCssVars, applyThemeMode, getThemeBySkin, resolveThemeSkin } from '/@/utils/themeSkin';
...
const skinVal = computed<ThemeSkin>({
	get: () => resolveThemeSkin(getThemeConfig.value.skin),
	set: (skin) => {
		getThemeConfig.value.skin = skin;
	},
});

const syncThemeAppearance = (primary?: string) => {
	const skin = resolveThemeSkin(getThemeConfig.value);
	applyThemeMode(skin, getThemeConfig.value.isIsDark);
	applyThemeCssVars(skin, {
		isDark: getThemeConfig.value.isIsDark,
		primary: primary || getThemeConfig.value.primary,
	});
};

// 一键换肤
const onChangeSkin = (skin: ThemeSkin) => {
	const resolvedSkin = resolveThemeSkin(skin);
	const theme = getThemeBySkin(resolvedSkin);

	storesThemeConfig.setSkin(resolvedSkin);
	syncThemeAppearance(theme.color.primary.normal);
...
const onColorPickerChange = () => {
	...
	syncThemeAppearance(getThemeConfig.value.primary);
	setDispatchThemeConfig();
};
// 2、菜单 / 顶栏
...
const onAddDarkChange = () => {
	isDark.value = getThemeConfig.value.isIsDark;
	syncThemeAppearance();
	setDispatchThemeConfig();
};
// 4、界面显示 --> 开启水印
