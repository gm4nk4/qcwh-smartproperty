<template>
  ...
  >
    <template v-for="val in menuLists">
      ...
      >
        <template #title>
          <i
            ...
            :class="{ 'is-empty': !hasMenuIcon(val) }"
          >
            <SvgIcon
              v-if="hasMenuIcon(val)"
              :name="val.meta.icon"
              :size="16"
              :color="getLeafIconColor(val)"
            />
          </i>
          <span class="layout-nav-menu__label" v-if="!state.isCollapse">{{
      ...
      <template v-else>
        <el-menu-item
          class="layout-nav-menu__leaf"
          :class="{ 'is-no-icon': !hasMenuIcon(val) }"
          :index="val.path"
          :key="val.path"
        >
          <i
  ...
  <div class="layout-nav-menu__footer">
    <Breadcrumb />
  </div>
</template>
...
const Breadcrumb = defineAsyncComponent(
  () => import("../navBars/breadcrumb/breadcrumb.vue"),
);
...
const sidebarCssVars = computed(() => ({
  ...
  "--layout-sidebar-menu-icon-active-color":
    sidebarTokens.value.activeIconColor,
...
const getLeafIconColor = (val: RouteItem) => {
  return isLeafActive(val)
    ? sidebarTokens.value.activeIconColor
    : sidebarTokens.value.iconColor;
};
...
.layout-nav-menu {
  ...
  :deep(.layout-nav-menu__leaf.is-active) {
    ...
    box-shadow: 0 25px 25px -15px rgba(0, 0, 0, 0.05);
  ...
  :deep(.layout-nav-menu__child-group > .el-sub-menu__title::before) {
    content: "";
    position: absolute;
    left: var(--before-left) !important;
    top: 50%;
    width: 6px;
    height: 24px;
    border-radius: 0 6px 6px 0;
  ...
  :deep(
    .layout-nav-menu__child-group.is-active
      > .el-sub-menu__title.is-active::before
  ) {
    background: var(--layout-sidebar-menu-marker-color);
    box-shadow: 0 20px 27px 0 rgba(0, 0, 0, 0.05);
  }
  ...
  :deep(
    .layout-nav-menu__child-group.is-opened.is-active > .el-sub-menu__title
  ),
  ...
  :deep(
    .layout-nav-menu__child-group > .el-sub-menu__title .layout-nav-menu__label
  ) {
    margin-left: 0;
  }
...
// 侧边栏折叠弹出菜单样式
:global(.el-popper .el-sub-menu__title),
:global(.el-popper .el-menu-item) {
  background: transparent !important;

  // 只给 当前 hover 的这一项 设置颜色
  &:hover {
    background: transparent !important;
  }
}
