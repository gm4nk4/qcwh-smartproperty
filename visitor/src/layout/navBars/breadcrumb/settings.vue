		...
		>
			<el-scrollbar class="layout-breadcrumb-seting-bar">
				...
				<div class="layout-breadcrumb-seting-bar-flex">
					<div class="layout-breadcrumb-seting-bar-flex-label">{{ $t('layout.fourIsOneClickSkin') }}</div>
					<div class="layout-breadcrumb-seting-bar-flex-value">
						<el-select v-model="skinVal" placeholder="请选择" style="width: 90px" @change="onChangeSkin">
							<el-option label="淡蓝皮肤" value="light-blue"></el-option>
							<el-option label="淡绿皮肤" value="light-green"></el-option>
						</el-select>
						<!-- <el-color-picker v-model="getThemeConfig.primary" @change="onColorPickerChange"> </el-color-picker> -->
					</div>
				</div>
				<!-- <div class="layout-breadcrumb-seting-bar-flex mt15">
					<div class="layout-breadcrumb-seting-bar-flex-label">{{ $t('layout.fourThemeColor') }}</div>
					<div class="layout-breadcrumb-seting-bar-flex-value">
						<el-color-picker v-model="getThemeConfig.primary" @change="onColorPickerChange" > </el-color-picker>
					</div>
				</div> -->
				...
				<div class="layout-breadcrumb-seting-bar-flex mt15">
					...
					<div class="layout-breadcrumb-seting-bar-flex-value">
						<el-select v-model="getThemeConfig.animation" placeholder="请选择" style="width: 90px" @change="setLocalThemeConfig">
							<el-option label="向右滑动" value="slide-right"></el-option>
							<el-option label="向左滑动" value="slide-left"></el-option>
							<el-option label="透明度变化" value="opacitys"></el-option>
						</el-select>
					</div>
				</div>
...
// 一键换肤
const onChangeSkin = () => {
	const theme = (themeConfigMap as Record<string, any>)[skinVal.value];
	if (!theme) return ElMessage.warning('未找到对应主题配置');

	// 更新全局主题颜色
	getThemeConfig.value.primary = theme.color.primary.normal;
	// 更新当前主题配置到 Pinia
	storesThemeConfig.setCurrentTheme(theme);

	// 颜色加深
	document.documentElement.style.setProperty('--el-color-primary-dark-2', `${getDarkColor(theme.color.primary.normal, 0.1)}`);
	document.documentElement.style.setProperty('--el-color-primary', theme.color.primary.normal);
	// 颜色变浅
	for (let i = 1; i <= 9; i++) {
		document.documentElement.style.setProperty(`--el-color-primary-light-${i}`, `${getLightColor(theme.color.primary.normal, i / 10)}`);
	}

	// 设置按钮 hover 和 disabled 状态颜色
	...
	// 设置文字颜色
	document.documentElement.style.setProperty('--theme-text-system', theme.color.text.system);
	document.documentElement.style.setProperty('--theme-text-dataAssist', theme.color.text.dataAssist);
	document.documentElement.style.setProperty('--theme-text-primaryTitle', theme.color.text.primaryTitle);
	document.documentElement.style.setProperty('--theme-text-assist', theme.color.text.assist);

	document.documentElement.style.setProperty(`--next-bg-topBarColor`, theme.color.primary.normal);
...
const onAddDarkChange = () => {
	...
	if (getThemeConfig.value.isIsDark) {
		...
		isDark.value = true;
		// 设置文字颜色
		document.documentElement.style.setProperty('--theme-text-system', '#ffffff');
		document.documentElement.style.setProperty('--theme-text-dataAssist', '#909399');
		document.documentElement.style.setProperty('--theme-text-primaryTitle', '#dadada');
		document.documentElement.style.setProperty('--theme-text-assist', '#606266');
	} else {
