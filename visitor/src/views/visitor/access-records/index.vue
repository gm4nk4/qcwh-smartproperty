		...
		<div class="layout-padding-auto layout-padding-view">
			...
			>
				...
				<template #statusSlot="{ row }">
					<span :class="getAccessRecordStatusMeta(row.status).className">
						{{ getAccessRecordStatusMeta(row.status).label }}
					</span>
				</template>
			</ConfigurableTableWithForm>
		</div>
...
import { getVisitorAccessRecordDetail, getVisitorAccessRecordPage, type VisitorAccessRecordDetail, type VisitorAccessRecordItem } from './api';
import {
...
import { buildAccessRecordDetailSections, buildAccessRecordPassRows, buildAccessRecordQueryParams, getAccessRecordStatusMeta } from './useBizProcess';

const tableRef = ref();

const queryParams = reactive<AccessRecordFormData>({ ...defaultFormData });
...
const loadTableData = async () => {
	...
	try {
		const res = await getVisitorAccessRecordPage(buildAccessRecordQueryParams(queryParams));
...
const handleSearch = (formData: Record<string, unknown>) => {
	...
	queryParams.licensePlate = String(formData.licensePlate ?? '');
	queryParams.visitedEnterprise = String(formData.visitedEnterprise ?? '');
	queryParams.visitedName = String(formData.visitedName ?? '');
	queryParams.passType = String(formData.passType ?? '');
	queryParams.passMethod = String(formData.passMethod ?? '');
	queryParams.appointmentNo = String(formData.appointmentNo ?? '');
	queryParams.passLocation = String(formData.passLocation ?? '');
...
const handleOperation = ({ action, row }: { action: string; row: VisitorAccessRecordItem }) => {
