<template>
	<div class="main-content">
		<!-- 左侧树形区域 -->
		<AppTree :tree-data="appTreeData" @node-click="handleNodeClick" />
				...
				>
					...
					<template #authPlatformRolesSlot="{ row }">
						<div class="platform-roles">
							<el-tag v-for="(role, index) in row.authPlatformRoles" :key="index" size="small" type="info" class="role-tag">
								{{ role }}
							</el-tag>
							<span v-if="row.authPlatformRoles.length === 0" class="no-data">--</span>
						</div>
					</template>
					<!-- 授权用户列插槽 -->
